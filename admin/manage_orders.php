...
if(isset($_POST['take_order'])) {
   //$row = mysqli_fetch_assoc($select_delb_id);
   $select_delb_id = 0;
   $order_update_id = $_POST['order_id'];
   $order_taken_id = $_POST['take_order'];

   $resultAll = mysqli_query($conn, "SELECT * FROM delivery_boy WHERE availability='YES' Limit 1");

   if (mysqli_num_rows($resultAll) > 0) {
      while($rowData = mysqli_fetch_array($resultAll)){
         $select_delb_id= $rowData['delb_id'];
...
if($select_delb_id>0){
      ...
      if($order_taken_id=='No'){
         ...
         mysqli_query($conn, "UPDATE `order_tbl` SET delb_id ='$select_delb_id' WHERE order_id = '$order_update_id'") or die('query failed');
         $message[] = 'Delivery boy has been assigned!';
         mysqli_query($conn, "UPDATE `delivery_boy` SET availability = 'No' WHERE delb_id = '$select_delb_id'") or die('query failed');

      }else{
         echo("Delivery boy already Added"); 
      }

}else{
  echo("No available delivery boys"); 
}
...
<section class="orders">
   ...
   <table>
      ...
      <?php
      $cur_date=date('Y-m-d');
      $select_orders = mysqli_query($conn, "SELECT * FROM `order_tbl`  ORDER BY order_taken, order_date") or die('query failed');
      ...
      <tr>
         ...
         <td><?php echo $fetch_orders['delb_id']; ?></td>
